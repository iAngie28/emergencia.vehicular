...
def agregar_horario(obj_in: HorarioTallerCreate, db: Session = Depends(deps.get_db)):
    """Define la hora de apertura y cierre para un día específico."""
    return horario_crud.create(db, obj_in=obj_in)
...
def crear_especialidad(obj_in: EspecialidadCreate, db: Session = Depends(deps.get_db)):
    ...
    return especialidad_crud.create(db, obj_in=obj_in)
...
def vincular_taller_con_especialidad(obj_in: TallerEspecialidadCreate, db: Session = Depends(deps.get_db)):
    """Asigna una especialidad a un taller con su nivel de experiencia."""
    return taller_especialidad_crud.create(db, obj_in=obj_in)
